...
Current operation is cloud-first: the GCP VM runs during the scheduled demo window; Windows holds project files and provides browser/CLI access. Use Grafana while the VM runs and exported CSV snapshots from GCS afterward. See [operations](docs/gcp_vm_operations.md) and [progress](docs/current_status.md).

Active local development environment (Docker) — a sandbox for logic and dashboard work, with alerting disabled; the cloud VM is the only environment that delivers alerts:
...
- It allows terminal SQL inside the VM checks to cross-check what Grafana shows.
...
| Cross-checking | SQL inside the VM can validate the same data Grafana displays. |
