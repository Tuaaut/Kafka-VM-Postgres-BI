...
Optional local development in WSL uses:
...
Optional private Grafana access from Windows PowerShell (uses the initialized SSH key):
...
Then open on the Windows workstation:
...
For operations-team alert emails, set the Grafana root URL to the public/reachable VM Grafana base URL, not the Windows workstation tunnel URL:
...
For cloud operations, SQL diagnostics inside the VM, and export review, use the [operations runbook](gcp_vm_operations.md). No desktop database client is required.
...
- Grafana dashboard opens from the Windows workstation.
