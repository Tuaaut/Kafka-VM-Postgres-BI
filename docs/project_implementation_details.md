...
- It supports SQL inside the VM checks.
...
| Offline review | VM exports PostgreSQL snapshots to GCS before shutdown, so the Windows workstation does not need to be online during the demo window. |
| Flexibility | PostgreSQL views and Grafana panels can evolve without changing the event stream. |
| Cross-checking | SQL inside the VM can validate Grafana dashboard values. |
...
- PostgreSQL is a Docker container. No native PostgreSQL installation is required on the Windows workstation.
- Use psql inside the running VM for occasional SQL diagnostics.
...
2. Monitor whether the Saturday 08:45-11:00 runtime is enough for normal demos.
...
6. Keep PostgreSQL private; use browser SSH and psql inside the running VM for occasional diagnostics.
