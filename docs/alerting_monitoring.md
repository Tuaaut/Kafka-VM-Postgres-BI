...
The public VM URL only works while the VM is running and Grafana is exposed. The SSH tunnel URL only works from the Windows workstation where the tunnel is open.
...
```text
Alert emails are for operations users. They cannot open a link to the owner's Windows workstation localhost.
