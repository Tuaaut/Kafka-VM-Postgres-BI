...
The email format is customized for operations users with summary, impact, action plan, dashboard link, silence link, and resolution note. Alert emails use `GRAFANA_ROOT_URL` so dashboard links point to the VM public Grafana URL instead of the owner's Windows workstation localhost.
...
The VM is not intended to run 24/7 for normal UAT. Compute Engine instance schedule `kafka-demo-uat-hours` starts it every Saturday at 08:45 and stops it at 11:00 Asia/Bangkok. The VM startup script starts Docker Compose automatically so the dashboard receives fresh producer data during the weekly demo window.
